...
-----------------------------------
Active TEKO robot (RL agent) + static RobotGoal with PBR ArUco marker.
"""
...
class TekoEnv(DirectRLEnv):
    """TEKO environment with one active robot and a static goal robot carrying an ArUco marker."""
    ...
    def _setup_scene(self):
        ...
        # --- Static goal robot -----------------------------------------
        TEKO_USD_PATH = "/workspace/teko/documents/CAD/USD/teko.usd"
        ARUCO_IMG_PATH = "/workspace/teko/documents/Aruco/4x4_1000-1.png"
        ...
        xf_goal.AddTranslateOp().Set(Gf.Vec3f(1.0, 0.0, 0.4))  # 40 cm above ground
        xf_goal.AddRotateZOp().Set(180.0)                      # face toward the arena center
        ...
        xf_aruco.AddTranslateOp().Set(Gf.Vec3f(0.1685, 0.0, -0.045))
        xf_aruco.AddRotateZOp().Set(180.0)

        primvars_api = UsdGeom.PrimvarsAPI(mesh)
        primvars_api.CreatePrimvar("st", Sdf.ValueTypeNames.TexCoord2fArray,
        ...
        tex = UsdShade.Shader.Define(stage, LOOKS_PATH + "/DiffuseTexture")
        ...
        tex_output = tex.CreateOutput("rgb", Sdf.ValueTypeNames.Float3)
        ...
        print("[INFO] RobotGoal + ArUco created, rotated, and textured successfully.")
    ...
    def _usd_randomize_robot_pose(self):
        """Randomize initial robot pose within a defined range."""
        stage = get_context().get_stage()
    ...
    # ------------------------------------------------------------------
    # Physics and observation methods
    # ------------------------------------------------------------------
