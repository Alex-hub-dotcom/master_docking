...
7. Yaw progress        - Reward for reducing yaw error
8. Collision penalty   - Terminal penalty for crashes
9. Boundary penalty    - Terminal penalty for leaving arena
10. Success bonus      - Terminal reward for docking (PROGRESSIVE THRESHOLD)
11. Time penalty       - Exponential penalty for slow episodes
...
# Import progressive tolerance from unified curriculum
from ..curriculum.curriculum_teko import get_success_threshold


# =============================================================================
# REWARD HYPERPARAMETERS (EXACTLY same as original v9.1)
# =============================================================================
...
# =============================================================================
# UTILITY FUNCTIONS (EXACTLY same as original)
# =============================================================================
...
def _get_success_distance(env) -> float:
    ...
    if hasattr(env, 'curriculum_level'):
        ...
        # Fallback to default
        return REWARD_CONFIG["success_distance_default"]


# =============================================================================
# REWARD COMPONENTS (EXACTLY same as original, except success uses progressive threshold)
# =============================================================================
...
def _compute_collision_penalty(env, surface_xy: torch.Tensor) -> torch.Tensor:
    ...
    # Use progressive success distance
    success_distance = _get_success_distance(env)
    raw_success = surface_xy < success_distance
...
def _compute_success_bonus(env, surface_xy: torch.Tensor) -> torch.Tensor:
    ...
    KEY CHANGE: Uses PROGRESSIVE success threshold based on curriculum stage.
    """
    cfg = REWARD_CONFIG
    
    # Use progressive success distance from curriculum
    success_distance = _get_success_distance(env)
...
def compute_total_reward(env) -> torch.Tensor:
    ...
    # ------------------------------------------------------------------
    # Logging (includes current success threshold for debugging)
    # ------------------------------------------------------------------
    _log_rewards(env, {
...
def check_success(env, surface_xy: torch.Tensor) -> torch.Tensor:
    """
    Check if docking is successful using progressive threshold.
    
    This function can be used externally to determine success
    with the correct stage-based threshold.
    
    Returns:
        Boolean tensor [num_envs] indicating success
    """
    cfg = REWARD_CONFIG
    success_distance = _get_success_distance(env)
    ep_len = env.episode_length_buf
    
    return (surface_xy < success_distance) & (ep_len >= cfg["success_min_steps"])


def get_current_success_threshold(env) -> float:
    """Get current success threshold based on curriculum stage."""
    return _get_success_distance(env)