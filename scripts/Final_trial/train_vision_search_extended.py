#!/usr/bin/env python3
"""
TEKO Vision Search EXTENDED - Maximize SSR
===========================================
Continues training from search_FINAL_S7.pt until reaching target SSR.
No stage advancement - stays at Stage 7 (full arena) and maximizes success rate.

Target: 99% SSR (or as high as possible)
...
# =============================================================================
# CONFIGURATION - Extended training for maximum SSR
# =============================================================================
CONFIG = {
    "max_steps": 500_000_000,  # 500M steps max
    "max_hours": 72,  # 3 days max
    
    # Hyperparameters from Optuna Trial 1 (same as before)
    "learning_rate": 0.000103,
    "entropy_coef": 0.00309,
    "gae_lambda": 0.94,
    "gamma": 0.99,
    "clip_ratio": 0.2,
    "value_coef": 0.5,
    ...
    # NO curriculum advancement - stay at Stage 7
    "fixed_stage": 7,
    
    # Target SSR to stop training
    "target_ssr": 0.99,  # 99%
    "min_episodes_for_target": 500,  # Need at least 500 episodes to confirm 99%
    
    # SSR thresholds to save checkpoints
    "ssr_save_thresholds": [0.80, 0.85, 0.90, 0.95, 0.97, 0.99],
    
    "log_interval": 50_000,
    "save_interval": 5_000_000,  # Regular saves every 5M steps
    
    # Checkpoint to load (Search S7)
    "pretrained_checkpoint": "/home/schux00/checkpoints/search_FINAL_S7.pt",
}


# =============================================================================
# NEURAL NETWORK (identical to original)
# =============================================================================
...
def train(args):
    ...
    # Import SEARCH curriculum
    from teko.tasks.direct.teko.curriculum.curriculum_manager_search import (
        reset_environment_curriculum_search,
        set_curriculum_level_search,
    )
    ...
    # Monkey-patch the environment to use search curriculum
    def patched_reset_idx(env_ids):
        ...
        # Use SEARCH curriculum
        reset_environment_curriculum_search(env, env_ids)
    ...
    env.set_curriculum_level = lambda level: set_curriculum_level_search(env, level)
    
    # SET TO STAGE 7 (full arena) and KEEP IT THERE
    env.set_curriculum_level(CONFIG["fixed_stage"])
    
    # Create policy
    policy = VisionIMUAttentionYawPolicy().to(device)
    
    # Load pretrained checkpoint (Search S7)
    start_step = 0
    if os.path.exists(CONFIG["pretrained_checkpoint"]):
        print(f"[LOAD] Loading checkpoint: {CONFIG['pretrained_checkpoint']}")
        checkpoint = torch.load(CONFIG["pretrained_checkpoint"], map_location=device)
        policy.load_state_dict(checkpoint["policy"])
        start_step = checkpoint.get("step", 0)
        print(f"[LOAD] Loaded policy from step {start_step:,}, stage {checkpoint.get('max_stage', 'unknown')}")
    else:
        print(f"[ERROR] Checkpoint not found: {CONFIG['pretrained_checkpoint']}")
        print("[ERROR] Cannot continue training without Search S7 checkpoint!")
        sys.exit(1)
    ...
    if os.path.exists(CONFIG["pretrained_checkpoint"]):
        ...
        if "optimizer" in checkpoint:
            try:
                ...
                print("[LOAD] Loaded optimizer state")
            except:
                print("[WARN] Could not load optimizer state, starting fresh")
    
    # Logging setup
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    log_dir = f"/home/schux00/tensorboard/search_extended_{timestamp}"
    csv_path = f"/home/schux00/logs/search_extended_{timestamp}.csv"
    ...
    csv_writer.writerow(['step', 'ssr', 'reward', 'entropy', 'yaw_loss', 'policy_loss', 'value_loss', 'hours', 'episodes'])
    ...
    # Use larger deque for more stable SSR measurement
    ep_rewards = deque(maxlen=1000)
    stage_successes = deque(maxlen=1000)
    cur_reward = torch.zeros(num_envs, device=device)
    
    # Track best SSR and thresholds reached
    best_ssr = 0.0
    ssr_thresholds_saved = set()
    total_episodes = 0
    
    obs_dict, _ = env.reset()
    step = start_step
    t0 = time.time()
    next_log = step + CONFIG["log_interval"]
    next_save = step + CONFIG["save_interval"]
    
    print("=" * 70)
    print("TEKO Vision SEARCH EXTENDED - Maximize SSR")
    print("=" * 70)
    print(f"Host: {socket.gethostname()}")
    print(f"Checkpoint: {CONFIG['pretrained_checkpoint']}")
    print(f"Starting step: {start_step:,}")
    print(f"Fixed Stage: {CONFIG['fixed_stage']} (full arena)")
    print(f"Target SSR: {CONFIG['target_ssr']:.0%}")
    print(f"Max hours: {CONFIG['max_hours']}")
    print(f"Envs: {num_envs} | LR: {CONFIG['learning_rate']}")
    ...
    try:
        while step < CONFIG["max_steps"]:
            ...
            # Track best SSR
            if ssr > best_ssr and len(stage_successes) >= 100:
                best_ssr = ssr
            
            # Save checkpoint at SSR thresholds
            for threshold in CONFIG["ssr_save_thresholds"]:
                if (ssr >= threshold and 
                    threshold not in ssr_thresholds_saved and 
                    len(stage_successes) >= 200):
                    
                    ssr_pct = int(threshold * 100)
                    ckpt_path = f"/home/schux00/checkpoints/search_extended_SSR{ssr_pct}.pt"
                    torch.save({
                        "step": step,
                        "ssr": ssr,
                        "best_ssr": best_ssr,
                    ...
                    print(f"[MILESTONE] SSR {ssr:.1%} reached! Saved to {ckpt_path}")
                    ssr_thresholds_saved.add(threshold)
            
            # Logging
            if step >= next_log:
                print(f"[{step:,}] SSR: {ssr:.1%} (best: {best_ssr:.1%}) | R: {mean_r:.1f} | "
                      f"YawL: {metrics['yaw_loss']:.3f} | Ent: {metrics['entropy']:.3f} | "
                      f"Ep: {total_episodes:,} | {elapsed_h:.1f}h")
                
                if writer:
                    writer.add_scalar("train/ssr", ssr, step)
                    writer.add_scalar("train/best_ssr", best_ssr, step)
                    ...
                    writer.add_scalar("train/episodes", total_episodes, step)
                
                csv_writer.writerow([step, f"{ssr:.4f}", f"{mean_r:.2f}",
                                    f"{metrics['entropy']:.4f}", f"{metrics['yaw_loss']:.4f}",
                                    f"{metrics['policy_loss']:.4f}", f"{metrics['value_loss']:.4f}",
                                    f"{elapsed_h:.2f}", total_episodes])
                csv_file.flush()
            ...
            # Regular checkpoint
            if step >= next_save:
                ckpt_path = f"/home/schux00/checkpoints/search_extended_{step//1000}k.pt"
                torch.save({
                    "step": step,
                    "ssr": ssr,
                    "best_ssr": best_ssr,
            ...
            # Check target SSR
            if (ssr >= CONFIG["target_ssr"] and 
                len(stage_successes) >= CONFIG["min_episodes_for_target"]):
                print("=" * 70)
                print(f"[SUCCESS] Target SSR {CONFIG['target_ssr']:.0%} reached!")
                print(f"[SUCCESS] Final SSR: {ssr:.1%} over {len(stage_successes)} episodes")
                print("=" * 70)
                break
        ...
        # Final save
        final_ssr_pct = int(best_ssr * 100)
        final_path = f"/home/schux00/checkpoints/search_extended_FINAL_SSR{final_ssr_pct}.pt"
        torch.save({
            "step": step,
            "ssr": ssr if 'ssr' in dir() else 0,
            "best_ssr": best_ssr,
            "episodes": total_episodes,
            "policy": policy.state_dict(),
            "optimizer": optimizer.state_dict(),
        ...
        print(f"[DONE] BestSSR={best_ssr:.1%}, Steps={step:,}, Episodes={total_episodes:,}, Time={(time.time()-t0)/3600:.1f}h")
