#!/usr/bin/env python3
"""
Continue Training T191 from Checkpoint
=======================================

Loads the T191 checkpoint (S49, 76% SSR) and continues training
until 95% SSR or 120h walltime.
...
# =============================================================================
CONFIG = {
    "checkpoint_path": "/home/schux00/checkpoints/BEST_T191_S49_76pct.pt",
    "max_walltime_s": 120 * 3600,  # 120h
    "target_ssr": 0.95,  # Stop if SSR >= 95%
    "log_interval": 50_000,
    "save_interval": 2_000_000,
    "num_envs": 120,
    "rollout_len": 128,
    
    # T191 hyperparameters
    "learning_rate": 0.00012262119179919,
    "entropy_coef": 0.006372155207986292,
    "gae_lambda": 0.94,
    "epochs": 3,
    "batch_size": 1024,
    "aux_yaw_coef": 0.38,
...
def main():
    ...
    # Load checkpoint
    print("=" * 70)
    print("CONTINUE T191 FROM CHECKPOINT")
    print("=" * 70)
    ...
    current_stage = ckpt.get("stage", 49)
    
    print(f"Loaded from step {start_step:,}, stage {current_stage}")
    print(f"Target: {CONFIG['target_ssr']:.0%} SSR at 1.0cm")
    print(f"Max walltime: {CONFIG['max_walltime_s'] // 3600}h")
    print("=" * 70)
    
    # Set to stage 49
    env.set_curriculum_level(49)
    success_threshold = get_success_threshold(49)
    ...
    csv_path = f"/home/schux00/logs/continue_T191_{timestamp}.csv"
    csv_file = open(csv_path, 'w', newline='')
    csv_writer = csv.writer(csv_file)
    csv_writer.writerow(['step', 'ssr', 'reward', 'entropy', 'yaw_loss', 'hours'])
    
    try:
        while True:
            elapsed_h = (time.time() - t0) / 3600
            total_elapsed_h = elapsed_h + (start_step / (num_envs * rollout_len * 3600 / 0.1))  # rough estimate
            
            if elapsed_h * 3600 > CONFIG["max_walltime_s"]:
                print(f"[TIME] Reached {CONFIG['max_walltime_s'] // 3600}h limit")
                break
            ...
            if ssr > best_ssr:
                ...
                ckpt_path = f"/home/schux00/checkpoints/T191_continued_BEST_{ssr*100:.0f}pct.pt"
                torch.save({
                    "step": step,
                    "stage": 49,
                ...
                print(f"[BEST] New best SSR: {ssr:.1%} -> {ckpt_path}")
            
            if ssr >= CONFIG["target_ssr"]:
                print("=" * 70)
                print(f"[SUCCESS] Reached {ssr:.1%} SSR at 1.0cm precision!")
                print("=" * 70)
                break
            
            # Logging
            if step >= next_log:
                print(f"[{step:,}] S49 | SSR: {ssr:.1%} (best: {best_ssr:.1%}) | R: {mean_r:.1f} | "
                      f"YawL: {metrics['yaw_loss']:.3f} | Ent: {metrics['entropy']:.3f} | {elapsed_h:.1f}h")
            ...
            # Checkpoint
            if step >= next_save:
                ckpt_path = f"/home/schux00/checkpoints/T191_continued_{step//1000}k.pt"
                torch.save({
                    "step": step,
                    "stage": 49,
                    "policy": policy.state_dict(),
        ...
        # Final save
        final_ssr = float(np.mean(stage_successes)) if stage_successes else 0.0
        final_path = f"/home/schux00/checkpoints/T191_continued_FINAL_{final_ssr*100:.0f}pct.pt"
        torch.save({
            "step": step,
            "stage": 49,
            "policy": policy.state_dict(),
        ...
        print(f"[DONE] Final SSR: {final_ssr:.1%}, Best SSR: {best_ssr:.1%}, Time: {elapsed_h:.1f}h")
