#!/usr/bin/env python3
"""
TEKO Vision Search - Arena-Wide Docking
========================================
Transfer learning from Stage 41 checkpoint.
Uses search curriculum for arena-wide spawning.

Loads: vision_optimal_FINAL_S41.pt
Curriculum: curriculum_manager_search.py
...
# =============================================================================
# CONFIGURATION - Fixed hyperparameters from Optuna Trial 1
# =============================================================================
CONFIG = {
    "max_steps": 200_000_000,
    "max_hours": 96,  # 4 days
    
    # Hyperparameters from Optuna Trial 1 (reached S41)
    ...
    "min_steps_before_advance": 300_000,  # Slightly longer for harder task
    "max_stage": 7,  # 8 stages (0-7)
    
    "log_interval": 50_000,
    "save_interval": 2_000_000,
    
    # Checkpoint to load
    "pretrained_checkpoint": "/home/schux00/checkpoints/vision_optimal_FINAL_S41.pt",
}
...
def train(args):
    ...
    # Import SEARCH curriculum (not the original!)
    ...
    def patched_reset_idx(env_ids):
        ...
        # Use SEARCH curriculum instead of original
    ...
    if os.path.exists(CONFIG["pretrained_checkpoint"]):
        print(f"[LOAD] Loading pretrained checkpoint: {CONFIG['pretrained_checkpoint']}")
        checkpoint = torch.load(CONFIG["pretrained_checkpoint"], map_location=device)
        policy.load_state_dict(checkpoint["policy"])
        print(f"[LOAD] Loaded policy from stage {checkpoint.get('max_stage', 'unknown')}")
    else:
        print(f"[WARN] Checkpoint not found: {CONFIG['pretrained_checkpoint']}")
    ...
    log_dir = f"/home/schux00/tensorboard/search_{timestamp}"
    csv_path = f"/home/schux00/logs/search_{timestamp}.csv"
    ...
    print("=" * 70)
    print("TEKO Vision SEARCH - Arena-Wide Docking")
    print("=" * 70)
    print(f"Host: {socket.gethostname()}")
    print(f"Pretrained: {CONFIG['pretrained_checkpoint']}")
    print(f"Envs: {num_envs} | Max Steps: {CONFIG['max_steps']:,}")
    print(f"LR: {CONFIG['learning_rate']} | Entropy: {CONFIG['entropy_coef']}")
    print(f"Stages: 0-{CONFIG['max_stage']} (arena-wide)")
    ...
    try:
        while step < CONFIG["max_steps"]:
            ...
            # Logging
            if step >= next_log:
                print(f"[{step:,}] S{current_stage} | SSR: {ssr:.1%} | R: {mean_r:.1f} | "
            ...
            # Checkpoint
            if step >= next_save:
                ckpt_path = f"/home/schux00/checkpoints/search_S{current_stage}_{step//1000}k.pt"
        ...
        final_path = f"/home/schux00/checkpoints/search_FINAL_S{max_stage_reached}.pt"
        torch.save({
            "step": step,
            "stage": current_stage,
            "max_stage": max_stage_reached,
