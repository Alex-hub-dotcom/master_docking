...
def objective(trial, env, base_log_dir, get_success_threshold):
    ...
    csv_writer.writerow(['step', 'stage', 'ssr', 'reward', 'entropy', 'yaw_loss', 'policy_loss', 
                        'value_loss', 'success_threshold_cm', 'hours'])
        ...
        while step < OPTUNA_CONFIG["max_steps_per_trial"]:
            ...
            for t in range(rollout_len):
                ...
                if done.any():
                    ...
                    # Use PROGRESSIVE success threshold
            ...
            if (len(stage_successes) >= 100 and
                ...
                # Update success threshold for new stage
                success_threshold = get_success_threshold(current_stage)
                
                print(f"[T{trial.number}][ADVANCE] Stage {current_stage-1} -> {current_stage} "
...
def run_worker(args):
    ...
    # Monkey-patch environment to use NEW unified curriculum
