...
"""
TEKO Optuna PPO v6 - 128x128 Vision + Auxiliary Yaw Head
========================================================

Key changes from v5:
- Resolution: 128x128 (was 84x84) for finer orientation details
- Auxiliary yaw head: CNN predicts yaw_error, supervised by ground truth
- This FORCES the CNN to learn orientation-relevant features
...
Architecture:
    CNN → features (256D)
      ...
      Actor    Yaw Head (NEW)
         ↓         ↓
      actions   yaw_pred → MSE(yaw_pred, yaw_true)

    Total Loss = PPO_loss + aux_coef * yaw_aux_loss
...
# =============================================================================
# CONFIG - 128x128 VISION + YAW AUXILIARY
# =============================================================================

OPTUNA_CONFIG = {
    "study_name": "teko_nsgaii_v6_128_yawaux",
    "storage_path": "/home/schux00/optuna/teko_nsgaii_v6_128_yawaux.db",

    "target_total_trials": 100,

    "max_steps_per_trial": 15_000_000,
    "max_walltime_s_per_trial": 14_400,  # 4h
...
FIXED_PARAMS = {
    ...
    "value_coef": 0.5,
    "max_grad_norm": 0.5,
    "aux_yaw_coef": 0.5,  # Weight for auxiliary yaw loss

    "num_envs": 96,  # Slightly fewer due to larger images
    "rollout_len": 64,
}
...
# =============================================================================
# VISION ENCODER (128x128)
# =============================================================================

class VisionEncoder(nn.Module):
    """CNN encoder for 128x128 grayscale frame stacks."""
    
    def __init__(self, in_channels: int = 4, feature_dim: int = 256):
        super().__init__()

        # Deeper network for 128x128
        self.conv = nn.Sequential(
            # 128 -> 42
            nn.Conv2d(in_channels, 32, kernel_size=8, stride=3, padding=1),
            nn.GroupNorm(8, 32),
...
# =============================================================================
# POLICY WITH AUXILIARY YAW HEAD
# =============================================================================

class VisionPolicyWithYawAux(nn.Module):
    """
    Vision policy with auxiliary yaw prediction head.
    
    The yaw head forces the CNN to learn orientation-relevant features
    by supervised learning on ground-truth yaw error.
    """
    ...
    def __init__(
        ...
        # ============================================================
        # AUXILIARY YAW HEAD - Predicts yaw_error from vision features
        # ============================================================
    ...
    def _init_heads(self):
        for module in [self.actor_head, self.state_encoder, self.critic_head, self.yaw_head]:
        ...
        nn.init.orthogonal_(self.yaw_head[-2].weight, gain=0.1)  # Small init for aux head
    ...
    def _predict_yaw(self, vision_feat: torch.Tensor) -> torch.Tensor:
        """Predict yaw error in [-pi, pi] from vision features."""
        return self.yaw_head(vision_feat).squeeze(-1) * math.pi
    ...
    def evaluate(
        ...
        """
        Evaluate actions and also return yaw prediction for auxiliary loss.
        
        Returns: (log_prob, value, entropy, yaw_pred)
        """
        vision = obs["rgb"]
        privileged = obs.get("privileged", None)
        ...
        # Auxiliary yaw prediction
        yaw_pred = self._predict_yaw(vision_feat)
...
def ppo_update_with_aux(
    policy: VisionPolicyWithYawAux,
    ...
    yaw_targets: torch.Tensor,  # Ground truth yaw errors
    epochs: int,
    batch_size: int,
    clip_ratio: float,
    entropy_coef: float,
    ...
    """PPO update with auxiliary yaw loss."""
    
    device = next(policy.parameters()).device
    T, N = obs_rgb_u8.shape[:2]
    total = T * N

    rgb_flat_u8 = obs_rgb_u8.view(total, NUM_FRAMES, IMG_SIZE, IMG_SIZE)
    ...
    yaw_flat = yaw_targets.view(total)
    ...
    for _ in range(epochs):
        for start in range(0, total, batch_size):
            ...
            mb_yaw_target = yaw_flat[mb_idx]

            log_prob, value, entropy, yaw_pred = policy.evaluate(mb_obs, mb_actions)
            ...
            # Auxiliary yaw loss (MSE on normalized yaw)
            yaw_aux_loss = F.mse_loss(yaw_pred, mb_yaw_target)
            ...
            total_aux_loss += yaw_aux_loss.item()
            num_updates += 1

    return {"yaw_aux_loss": total_aux_loss / max(num_updates, 1)}
...
def objective(trial: optuna.Trial, env) -> Tuple[float, float]:
    """Train 128x128 vision PPO with auxiliary yaw head."""
    ...
    epochs = trial.suggest_int("epochs", 4, 7)
    learning_rate = trial.suggest_float("learning_rate", 5e-5, 1.5e-4, log=True)
    batch_size = trial.suggest_categorical("batch_size", [1024, 2048])
    aux_yaw_coef = trial.suggest_float("aux_yaw_coef", 0.2, 0.8)

    device = torch.device("cuda:0")
    env.set_curriculum_level(0)

    policy = VisionPolicyWithYawAux(
        vision_channels=NUM_FRAMES,
        privileged_dim=7,
        action_dim=2,
        hidden_dim=256
    ).to(device)
    ...
    yaw_targets = torch.empty((rollout_len, num_envs), device=device, dtype=torch.float32)
        ...
        while step < max_steps and (time.time() - t0) < max_wall_s:
            for t in range(rollout_len):
                ...
                if has_privileged:
                    ...
                    # Extract yaw_error (index 3) for auxiliary supervision
                    yaw_targets[t].copy_(obs["privileged"][:, 3])
            ...
            if step >= next_eval:
                ...
                yaw_loss = aux_stats.get("yaw_aux_loss", 0.0)
                print(f"[{step:,}] S{current_stage:02d} | SSR: {ssr:.1%} | R: {mean_reward:.1f} | YawL: {yaw_loss:.3f} | MaxS: {max_stage} | {elapsed/60:.1f}min")
                next_eval += eval_interval
                ...
                if OPTUNA_CONFIG["pruning_enabled"] and step >= OPTUNA_CONFIG["pruning_warmup_steps"]:
                    ...
                    if bad:
                        bad_eval_streak += 1
                        print(f"[PRUNE-CHECK] streak={bad_eval_streak} | SSR={ssr:.1%} < {min_ssr:.0%}")
                    else:
                        bad_eval_streak = 0
...
def run_worker(args):
    ...
    # REWARD_CONFIG uses v10 defaults (fine yaw focus)
...
def main():
    parser = argparse.ArgumentParser(description="TEKO v6 - 128x128 + Yaw Aux")
