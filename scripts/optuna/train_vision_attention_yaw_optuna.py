#!/usr/bin/env python3
"""
TEKO Vision + Attention + YawAux - Optuna NSGA-II
Best of both: Attention + YawAux combined
"""
...
OPTUNA_CONFIG = {
    "study_name": "teko_vision_attn_yaw_v1",
    "storage_path": "/home/schux00/optuna/teko_vision_attn_yaw_v1.log",
    "target_total_trials": 200,
    "max_steps_per_trial": 200_000_000,
    "max_walltime_s_per_trial": 604800,  # 5h
...
def objective(trial, env):
    ...
    priv_buf = torch.zeros((rollout_len, num_envs, PRIVILEGED_DIM), device=device) if has_priv else None
    ...
    try:
        while step < OPTUNA_CONFIG["max_steps_per_trial"] and (time.time() - t0) < OPTUNA_CONFIG["max_walltime_s_per_trial"]:
            ...
            if step >= next_eval:
                ...
                print(f"[{step:,}] S{current_stage:02d} | SSR: {ssr:.1%} | "
...
def run_worker(args):
    ...
    print("=" * 70)
    print("TEKO Vision + Attention + YawAux - Optuna NSGA-II")
    print("=" * 70)
...
def main():
    ...
    args.seed = 42  # Fixed seed for reproducibility
    if True:
