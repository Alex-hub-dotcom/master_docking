...
"""
TEKO Optuna State-Based Training (NSGA-II) v6
==============================================

Baseline training using privileged state instead of vision.
Purpose: verify that curriculum and rewards work correctly.

If state-based learns but vision doesn't -> problem is CNN/visual features.
If both fail -> problem is curriculum/rewards.

Same hyperparameters and Optuna structure as vision version.
...
OPTUNA_CONFIG = {
    "study_name": "teko_state_v9",
    "storage_path": "/home/schux00/optuna/teko_state_v9.db",

    "target_total_trials": 200,  # Fewer trials needed for state-based

    "max_steps_per_trial": 10_000_000,
    "max_walltime_s_per_trial": 5 * 24 * 3600,  # 5 days (faster than vision)
    ...
    # Higher thresholds for state-based (should learn faster)
    "min_ssr_thresholds": {
        0: 0.70,
        4: 0.60,
        8: 0.50,
...
FIXED_PARAMS = {
    ...
    "value_coef": 0.5,
    "max_grad_norm": 0.5,
    "num_envs": 120,      # More envs since no camera overhead
    "rollout_len": 128,
}

# Curriculum advancement thresholds (higher for state-based)
...
def create_study(study_name: str, storage) -> optuna.Study:
    study = optuna.create_study(
        ...
        sampler=optuna.samplers.NSGAIISampler(
            population_size=15,  # Smaller population for state-based
            mutation_prob=0.1,
            crossover_prob=0.9,
            seed=42,
        ),
...
# =============================================================================
# MLP POLICY (for state-based)
# =============================================================================

class MLPPolicy(nn.Module):
    """
    Simple MLP policy for state-based training.
    
    Input: 7D privileged state [dx, dy, dz, yaw_err, vx, vy, omega]
    ...
    def __init__(
        ...
        # Critic head (separate for asymmetric training if needed)
...
def ppo_update(
    ...
    """Perform PPO policy update for state-based policy."""
...
def objective(trial: optuna.Trial, env) -> Tuple[float, float]:
    """
    Optuna objective for state-based training.
    
    Tuned hyperparameters (same ranges as vision for fair comparison):
    ...
    # Rollout buffers
    obs_states = torch.empty((rollout_len, num_envs, 7), device=device, dtype=torch.float32)
...
def run_worker(args):
    ...
    # Import state-based environment
    ...
    print("=" * 70)
    print(f"TEKO State-Based Training v6 - Worker {slurm_array}")
    print("=" * 70)
...
def main():
    parser = argparse.ArgumentParser(description="TEKO State-Based Optuna Training")
